...
export default function StageLight({ position = [0, 0, 0], color = "#FFD700", intensity = 1, tier = 2 }) {
    ...
    return (
        <group position={position} ref={lightRef}>
            {/* Light Housing */}
            <Cylinder args={[0.8, 1.2, 2, isLowTier ? 6 : 8]} rotation={[Math.PI / 2, 0, 0]}>
                <meshStandardMaterial color="#222" metalness={0.9} roughness={0.1} />
            </Cylinder>

            {/* Lens */}
            <Cylinder args={[0.7, 0.7, 0.2, isLowTier ? 8 : 16]} position={[0, 0, 1.1]} rotation={[Math.PI / 2, 0, 0]}>
                <meshStandardMaterial color={color} emissive={color} emissiveIntensity={intensity} transparent opacity={0.8} />
            </Cylinder>

            {/* Light Beam */}
            <Cylinder ref={beamRef} args={[0.7, 8, 40, isLowTier ? 8 : 12]} position={[0, 0, 21]} rotation={[Math.PI / 2, 0, 0]}>
                <meshBasicMaterial color={color} transparent opacity={0.15} depthWrite={false} />
            </Cylinder>

            {/* Mounting Bracket - Hide on low tier */}
            {!isLowTier && (
            ...
            {/* Point Light - Disable on low tier */}
            {!isLowTier && (
